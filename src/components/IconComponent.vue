<template>
  <div id="IconComponent">
    ...
    >
      <component :is="icon" :size="iconSize" :fillCollor="iconColor" />
    </div>
    <div
      :id="`tooltip-no-arrow-${iconString}`"
      role="tooltip"
      class="inline-block absolute invisible text-xs z-10 py-1 px-2 font-medium text-white rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-600 delay-150 black"
    >
      {{ text }}
    </div>
  </div>
...
import TuneIcon from "vue-material-design-icons/Magnify.vue";
